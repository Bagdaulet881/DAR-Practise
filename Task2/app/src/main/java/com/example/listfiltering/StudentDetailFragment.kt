...
//        name = findViewById<View>(R.id.name) as TextView
        ...
        view.lastname.setText(student.lastname)
        view.email.setText(student.email)
        view.textView7.setText(student.age)
        view.textView8.setText(student.city)
        view.textView9.setText(student.university)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.textView9?.setOnClickListener(){
            Toast.makeText(getContext(), "Students of ${student.university}", Toast.LENGTH_SHORT).show()
            Data.savedKeyWord = student.university
            val fragmentBack: Fragment = BlankFragment(data.sortingBy(student.university))
            fragmentManager?.beginTransaction()?.replace(R.id.main_container, fragmentBack)?.commitAllowingStateLoss()
        }
    }
